...
After a successful mutation through `install`, the result names its scope: `Installed <packageId> in user scope`, `Updated <packageId> in system scope`, or `Installed <packageId> in project scope (~/project)`. A project path is canonical and displayed relative to canonical `HOME` with `~/` when possible, or as an absolute path otherwise. Skipped, unchanged, and failed installs retain their existing output. Updates performed by the separate `update` command also retain their existing output.
